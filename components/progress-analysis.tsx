...
export function ProgressAnalysisView({ data, onClose, onTrain, onGuide, initialDate, initialPeriod = "week" }: { data: TrainingData; onClose: () => void; onTrain: () => void; onGuide: () => void; initialDate?: string; initialPeriod?: ReviewPeriod }) {
  ...
  return <section className="progress-analysis motion-page" aria-label="Progress analysis">
    ...
    <div className="analysis-content motion-pop" key={tab}>
      ...
      {tab === "Muscles" && <section className="analysis-card"><h2>Where the work went</h2><p className="analysis-muted">By actual training date. Direct and supporting sets stay separate; a set can involve several muscles. These counts are not measurements of growth or an ideal weekly dose.</p><div className="analysis-legend"><span><i className="analysis-dot" data-status="on-track" />Targets met</span><span><i className="analysis-dot" data-status="hold" />Review targets</span><span><i className="analysis-dot" data-status="adjust" />Recovery concern</span><span><i className="analysis-dot" data-status="insufficient" />Missing information</span></div><MuscleAnatomy review={review} selected={selectedMuscle} select={setSelectedMuscle} /><div className="analysis-muscle-detail" aria-live="polite"><h3>{MUSCLE_LABELS[muscle.muscle]}</h3><Badge status={muscle.status} /><p><strong>{muscle.direct}</strong> direct sets · <strong>{muscle.indirect}</strong> supporting sets · {muscle.planned} direct sets in recorded plans</p><p>{muscle.reason}</p></div><p className="analysis-muted">Colors summarize muscle groups, not individual muscles or left/right differences. Pale areas are not tracked separately. Red does not diagnose an injury; green does not certify that training is safe. Missing workouts are not assigned invented muscle volume.</p></section>}
      {tab === "Full review" && <>
