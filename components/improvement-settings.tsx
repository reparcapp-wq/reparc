...
export function ImprovementSettings({ onContinue }: { onContinue?: () => void }) {
  ...
  return <section className="space-y-4 text-sm leading-6">
    ...
    <p className="text-stone-300">Let RepArc evaluate its recommendations using future exercise targets, completed sets, loads, reps, effort, recovery, program track and lifting experience. Saying no does not change your program.</p>
    <p className="text-stone-400">Records are linked to your account, with no name or email in the evaluation copy. Authorized RepArc maintainers may review them. Records expire after 180 days; consent and access records after 365 days. Participation expires after one year.</p>
    <p className="text-stone-400">Withdraw here to stop sharing and delete the evaluation copy. Your own workout history stays. No advertising, automatic algorithm training or third-party analytics. <a href="/privacy" target="_blank" rel="noreferrer" className="text-amber-300 underline">Read the privacy notice</a>.</p>
    <label className="flex min-h-12 cursor-pointer items-start gap-3 rounded-xl border border-white/15 p-4"><Checkbox checked={chosen} disabled={busy || !status?.available} onCheckedChange={(value) => setChosen(value === true)} className="mt-1" /><span>I agree to this optional use of my future training and recovery data.</span></label>
